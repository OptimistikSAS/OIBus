...
describe('South item repository', () => {
  ...
  it('should create and update south items', () => {
    ...
    repository.createAndUpdateSouthItems('connectorId', [itemToAdd], [itemToUpdate], false);
    expect(database.prepare).toHaveBeenCalledWith(
      `INSERT INTO south_items (id, name, enabled, connector_id, scan_mode_id, settings) VALUES (?, ?, ?, ?, ?, ?);`
    );
    expect(database.prepare).toHaveBeenCalledWith(
  ...
  it('should create a copy of south items', () => {
    ...
    repository.createAndUpdateSouthItems('connectorId', [itemToAdd], [itemToUpdate], true);
    expect(database.prepare).toHaveBeenCalledWith(
      `INSERT INTO south_items (id, name, enabled, connector_id, scan_mode_id, settings) VALUES (?, ?, ?, ?, ?, ?);`
    );
    expect(database.prepare).toHaveBeenCalledWith(
