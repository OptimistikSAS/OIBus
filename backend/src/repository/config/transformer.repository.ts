...
export default class TransformerRepository {
  ...
  findAll(): Array<Transformer> {
    const query = `SELECT id, name, description, type, input_type, output_type, custom_manifest, custom_code FROM ${TRANSFORMERS_TABLE};`;
  ...
  save(transformer: CustomTransformer): void {
    if (!transformer.id) {
      ...
      this.database
        .prepare(
          `INSERT INTO ${TRANSFORMERS_TABLE} (id, name, description, type, input_type, output_type, custom_manifest, custom_code) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
        )
        .run(
          transformer.id,
          transformer.name,
  ...
  findById(id: string): Transformer | null {
    const query = `SELECT id, name, description, type, input_type, output_type, custom_manifest, custom_code FROM ${TRANSFORMERS_TABLE} WHERE id = ?;`;
  ...
  createStandardTransformer(transformer: StandardTransformer): void {
    this.database
      .prepare(`INSERT INTO ${TRANSFORMERS_TABLE} (id, name, description, type, input_type, output_type) VALUES (?, ?, ?, ?, ?, ?)`)
      .run(transformer.id, transformer.name, transformer.description, transformer.type, transformer.inputType, transformer.outputType);
  }
  ...
  search(searchParams: TransformerSearchParam): Page<Transformer> {
    ...
    const query =
      `SELECT id, name, description, type, input_type, output_type, custom_manifest, custom_code FROM ${TRANSFORMERS_TABLE} ${whereClause}` +
  ...
  private createStandardTransformers() {
    if (!this.findById(IsoTransformer.transformerName)) {
