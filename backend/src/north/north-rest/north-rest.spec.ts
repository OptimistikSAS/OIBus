...
describe.each(testCases)('NorthREST %s', (_, settings) => {
  ...
  it('should properly throw fetch error with files', async () => {
    ...
    await expect(north.handleContent({ type: 'raw', filePath })).rejects.toThrow(
      new OIBusError(`Failed to reach file endpoint ${endpoint}; message: error`, true)
    );
  ...
  it('should properly get message from generic Error', async () => {
    ...
    await expect(north.handleContent({ type: 'raw', filePath })).rejects.toThrow(
      new OIBusError(`Failed to reach file endpoint ${endpoint}; message: generic error object`, true)
    );
  ...
  it('should properly get message from non Errors', async () => {
    ...
    await expect(north.handleContent({ type: 'raw', filePath })).rejects.toThrow(
      new OIBusError(`Failed to reach file endpoint ${endpoint}; {"some":"data"}`, true)
    );
  ...
  it('should properly get message from generic AggregateError', async () => {
    ...
    await expect(north.handleContent({ type: 'raw', filePath })).rejects.toThrow(
      new OIBusError(`Failed to reach file endpoint ${endpoint}; message: error 1; message: error 2`, true)
    );
  ...
  it('should properly get message from Error with code', async () => {
    ...
    await expect(north.handleContent({ type: 'raw', filePath })).rejects.toThrow(
      new OIBusError(`Failed to reach file endpoint ${endpoint}; message: error with code, code: 1`, true)
    );
  ...
  if (settings.useProxy) {
    it('should throw error when proxy url is not defined', async () => {
      ...
      const filePath = '/path/to/file/example.file';
      await expect(north.handleContent({ type: 'raw', filePath })).rejects.toThrow(
        `Failed to reach file endpoint ${endpoint}; message: Proxy URL not specified`
      );
    });
  }
});
