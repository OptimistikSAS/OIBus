...
describe.each(testCases)('NorthOIAnalytics %s', (_, settings) => {
  ...
  it('should properly throw fetch error with values', async () => {
    ...
    await expect(north.handleValues(values)).rejects.toThrow(
      new OIBusError(`Fail to reach values endpoint ${hostname}/api/oianalytics/oibus/time-values. ${error}`, true)
    );
  });
  ...
  it('should properly throw error on values bad response', async () => {
    ...
    await expect(north.handleValues(values)).rejects.toThrow(new OIBusError(`Error 400: "statusText"`, false));
  ...
  it('should properly throw error when file does not exist', async () => {
    ...
    await expect(north.handleContent({ type: 'raw', filePath })).rejects.toThrow(`File ${filePath} does not exist`);
  });
...
describe('NorthOIAnalytics with OIA module', () => {
  ...
  it('should not use oia module if not registered', async () => {
    ...
    (oIAnalyticsRegistrationRepository.get as jest.Mock).mockReturnValue(registrationSettings);
    const filePath = '/path/to/file/example.file';
    await expect(north.handleContent({ type: 'raw', filePath })).rejects.toThrow(new Error('OIBus not registered in OIAnalytics'));
  });
