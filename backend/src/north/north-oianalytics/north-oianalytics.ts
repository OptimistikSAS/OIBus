...
export default class NorthOIAnalytics extends NorthConnector<NorthOIAnalyticsSettings> {
  ...
  async handleValues(values: Array<OIBusTimeValue>): Promise<void> {
    ...
    if (!response.ok) {
      throw {
        message: `Error ${response.statusCode}: ${await response.body.text()}`,
        retry: [401, 403, 404, 500, 502, 503, 504].includes(response.statusCode)
      };
    }
  }
  ...
  async handleFile(filePath: string): Promise<void> {
    ...
    if (!response.ok) {
      throw new OIBusError(
        ...
        [400, 401, 403, 404, 500, 502, 503, 504].includes(response.statusCode)
      );
    }
  }
