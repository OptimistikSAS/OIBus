...
export default abstract class NorthConnector<T extends NorthSettings> {
  ...
  async cacheContent(data: OIBusContent, _source: string): Promise<void> {
    ...
    if (data.type === 'raw') {
      ...
      await this.cacheSingleBatch(readStream, cacheFilename, 'raw', 0, randomId);
    } else {
      if (this.connector.caching.throttling.maxNumberOfElements > 0) {
        for (let i = 0; i < data.content.length; i += this.connector.caching.throttling.maxNumberOfElements) {
          ...
          await this.cacheSingleBatch(readStream, cacheFilename, data.type, chunks.length, randomId);
        }
      } else {
        const randomId = generateRandomId(10);
        const cacheFilename = `${randomId}.json`;
        const readStream = Readable.from(JSON.stringify(data.content));
        await this.cacheSingleBatch(readStream, cacheFilename, data.type, data.content.length, randomId);
      }
    }
