...
export default abstract class NorthConnector<T extends NorthSettings> {
  ...
  async cacheContent(data: OIBusContent, source: string): Promise<void> {
    ...
    if (data.type === 'raw') {
      ...
      // Prevent parent paths injected into the json file in cache by checking the start of the path: it must be in the data folder
      if (!resolvedPath.startsWith(path.resolve(this.cacheService.cacheFolder, '..'))) {
      ...
      if (this.connector.caching.throttling.maxNumberOfElements > 0) {
        for (let i = 0; i < data.content.length; i += this.connector.caching.throttling.maxNumberOfElements) {
          ...
          await this.cacheSingleBatch(readStream, data.type, source);
        }
      } else {
  ...
  private async cacheSingleBatch(readStream: ReadStream | Readable, inputType: string, source: string, filename = '') {
    const randomId = generateRandomId(10);
    // Either generate a filename
    const cacheFilename = filename ? `${path.parse(filename).name}-${randomId}${path.parse(filename).ext}` : `${randomId}.json`;
    ...
    const { metadata, output } = await transformer.transform(readStream, source, cacheFilename);
