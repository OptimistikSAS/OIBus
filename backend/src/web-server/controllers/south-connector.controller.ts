...
export default class SouthConnectorController {
  ...
  async findById(ctx: KoaContext<void, SouthConnectorDTO<SouthSettings, SouthItemSettings>>): Promise<void> {
    ...
    ctx.ok(toSouthConnectorDTO(southConnector, manifest, ctx.app.encryptionService));
  }
  ...
  async createSouth(
    ...
    try {
      const manifest = ctx.app.southService
        .getInstalledSouthManifests()
        .find(southManifest => southManifest.id === ctx.request.body!.type)!;
      const southConnector = toSouthConnectorDTO(
        await ctx.app.southService.createSouth(ctx.request.body!),
        manifest,
  ...
  async resetSouthMetrics(ctx: KoaContext<void, void>): Promise<void> {
    const southConnector = ctx.app.repositoryService.southConnectorRepository.findSouthById(ctx.params.southId);
    if (southConnector) {
  ...
  async testSouthConnection(ctx: KoaContext<SouthConnectorCommandDTO<SouthSettings, SouthItemSettings>, void>): Promise<void> {
    try {
      ...
      await ctx.app.southService.testSouth(ctx.params.id, ctx.request.body!, manifest, logger);
      ctx.noContent();
    } catch (error: unknown) {
      ctx.badRequest((error as Error).message);
    }
  ...
  async testSouthItem(
    ...
    try {
      ...
      await ctx.app.southService.testSouthItem(ctx.params.id, ctx.request.body!.south, ctx.request.body!.item, manifest, ctx.ok, logger);
    } catch (error: unknown) {
      ctx.badRequest((error as Error).message);
    }
  }
  ...
  async listSouthItems(ctx: KoaContext<void, Array<SouthConnectorItemDTO<SouthItemSettings>>>): Promise<void> {
    ...
    const southItems = ctx.app.southService
      ...
      .map(item => toSouthConnectorItemDTO(item, manifest, ctx.app.encryptionService));
    ctx.ok(southItems);
  }
  ...
  async searchSouthItems(ctx: KoaContext<void, Page<SouthConnectorItemDTO<SouthItemSettings>>>): Promise<void> {
    ...
    ctx.ok({
      content: page.content.map(item => toSouthConnectorItemDTO(item, manifest, ctx.app.encryptionService)),
  ...
  async getSouthItem(ctx: KoaContext<void, SouthConnectorItemDTO<SouthItemSettings>>): Promise<void> {
    ...
    if (item) {
      ctx.ok(toSouthConnectorItemDTO(item, manifest, ctx.app.encryptionService));
    } else {
      ctx.notFound();
    }
  }
  ...
  async createSouthItem(
    ...
    try {
      ...
      const manifest = ctx.app.southService.getInstalledSouthManifests().find(southManifest => southManifest.id === southConnector.type)!;
      ctx.created(toSouthConnectorItemDTO(item, manifest, ctx.app.encryptionService));
    } catch (error: unknown) {
      ctx.badRequest((error as Error).message);
    }
  }
  ...
  async exportSouthItems(ctx: KoaContext<{ delimiter: string }, string>): Promise<void> {
    ...
    ctx.body = itemToFlattenedCSV(
      ctx.app.southService.findAllItemsForSouthConnector(ctx.params.southId).map(item => {
        return toSouthConnectorItemDTO(item, manifest, ctx.app.encryptionService);
      }),
      ctx.request.body!.delimiter
    );
    ctx.set('Content-disposition', 'attachment; filename=items.csv');
