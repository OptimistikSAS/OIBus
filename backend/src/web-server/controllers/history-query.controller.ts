...
export default class HistoryQueryController extends AbstractController {
  ...
  async findById(ctx: KoaContext<void, HistoryQueryDTO<SouthSettings, NorthSettings, SouthItemSettings>>): Promise<void> {
    ...
    return ctx.ok(toHistoryQueryDTO(historyQuery, northManifest, southManifest, ctx.app.encryptionService));
  }
  ...
  async createHistoryQuery(
    ...
    try {
      const southManifest = ctx.app.southService
        .getInstalledSouthManifests()
        .find(manifest => manifest.id === ctx.request.body!.southType)!;
      const northManifest = ctx.app.northService
        .getInstalledNorthManifests()
        .find(manifest => manifest.id === ctx.request.body!.northType)!;
  ...
  async testSouthConnection(ctx: KoaContext<SouthConnectorCommandDTO<SouthSettings, SouthItemSettings>, void>) {
    try {
      ...
      await ctx.app.historyQueryService.testSouth(ctx.params.id, ctx.request.body!, manifest, logger);
      ctx.noContent();
    } catch (error: unknown) {
      ctx.badRequest((error as Error).message);
    }
  ...
  async testNorthConnection(ctx: KoaContext<NorthConnectorCommandDTO<NorthSettings>, void>) {
    try {
      ...
      await ctx.app.historyQueryService.testNorth(ctx.params.id, ctx.request.body!, manifest, logger);
      ctx.noContent();
    } catch (error: unknown) {
      ctx.badRequest((error as Error).message);
    }
  ...
  async searchHistoryQueryItems(ctx: KoaContext<void, Page<HistoryQueryItemDTO<SouthItemSettings>>>): Promise<void> {
    const historyQuery = ctx.app.historyQueryService.findById(ctx.params.historyId);
    if (!historyQuery) {
      return ctx.notFound();
    }
    ...
    ctx.ok({
      content: page.content.map(item => toHistoryQueryItemDTO(item, manifest, ctx.app.encryptionService)),
  ...
  async getHistoryQueryItem(ctx: KoaContext<void, HistoryQueryItemDTO<SouthItemSettings>>): Promise<void> {
    ...
    if (historyQueryItem) {
      ctx.ok(toHistoryQueryItemDTO(historyQueryItem, manifest, ctx.app.encryptionService));
    } else {
      ctx.notFound();
    }
  }
  ...
  async createHistoryQueryItem(
    ...
    try {
      ...
      const manifest = ctx.app.southService.getInstalledSouthManifests().find(manifest => manifest.id === historyQuery.southType)!;
      ctx.created(toHistoryQueryItemDTO(item, manifest, ctx.app.encryptionService));
    } catch (error: unknown) {
      ctx.badRequest((error as Error).message);
    }
  }
  ...
  async exportSouthItems(ctx: KoaContext<{ delimiter: string }, string>): Promise<void> {
    ...
    ctx.body = itemToFlattenedCSV(
      ctx.app.historyQueryService.findAllItemsForHistoryQuery(ctx.params.historyQueryId).map(item => {
        return toHistoryQueryItemDTO(item, manifest, ctx.app.encryptionService);
      }),
      ctx.request.body!.delimiter
    );
    ctx.set('Content-disposition', 'attachment; filename=items.csv');
