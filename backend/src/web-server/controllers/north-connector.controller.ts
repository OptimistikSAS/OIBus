...
export default class NorthConnectorController {
  ...
  async findById(ctx: KoaContext<void, NorthConnectorDTO<NorthSettings>>): Promise<void> {
    ...
    if (northConnector) {
      const manifest = ctx.app.northService.getInstalledNorthManifests().find(southManifest => southManifest.id === northConnector.type)!;
      ctx.ok(toNorthConnectorDTO(northConnector, manifest, ctx.app.encryptionService));
    } else {
      ctx.notFound();
    }
  }
  ...
  async testNorthConnection(ctx: KoaContext<NorthConnectorCommandDTO<NorthSettings>, void>): Promise<void> {
    try {
      ...
      await ctx.app.northService.testNorth(ctx.params.id, ctx.request.body!, manifest, logger);
      ctx.noContent();
    } catch (error: unknown) {
      ctx.badRequest((error as Error).message);
    }
