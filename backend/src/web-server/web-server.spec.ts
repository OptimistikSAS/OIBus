...
type HomeMetricsMockType = { getHomeMetrics: ReturnType<typeof mock.fn> };
...
describe('WebServer', () => {
  ...
  it('start() should be a no-op if app is null (before init)', async () => {
    await assert.doesNotReject(() => (webServer as unknown as { start: () => Promise<void> }).start());
  });
  ...
  describe('error handling middleware', () => {
    it('handleBodyParserErrors: SyntaxError with body → 422, other → next', async () => {
      await webServer.init();
      type ErrFn = (err: unknown, req: unknown, res: { status: (c: number) => { json: (b: unknown) => void }; end: () => void }, next: (e?: unknown) => void) => void;
    ...
    it('setupErrorHandling: handles various error types correctly', async () => {
      await webServer.init();
      type SetupFn = () => (err: Error, req: unknown, res: { status: (c: number) => { json: (b: unknown) => void } }, next: unknown) => void;
