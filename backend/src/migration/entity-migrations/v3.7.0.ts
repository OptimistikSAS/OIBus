...
async function updateOleDBSettings(knex: Knex) {
  const oldSouth: Array<{
    id: string;
    type: string;
    settings: string;
  }> = await knex(SOUTH_CONNECTORS_TABLE).select('id', 'type', 'settings').where('type', 'oledb');

  for (const connector of oldSouth) {
    const oldSettings = JSON.parse(connector.settings) as OldSouthOLEDBSettings;
    const newSettings: NewSouthOLEDBSettings = { ...oldSettings, password: null };
    await knex(SOUTH_CONNECTORS_TABLE)
      .update({ settings: JSON.stringify(newSettings) })
      .where('id', connector.id);
  }
  ...
  const oldHistory: Array<{
    ...
    south_type: string;
    south_settings: string;
  }> = await knex(HISTORY_QUERIES_TABLE).select('id', 'south_type', 'south_settings').where('south_type', 'oledb');

  for (const history of oldHistory) {
    const oldSettings = JSON.parse(history.south_settings) as OldSouthOLEDBSettings;
    const newSettings: NewSouthOLEDBSettings = { ...oldSettings, password: null };
    await knex(HISTORY_QUERIES_TABLE)
      .update({ south_settings: JSON.stringify(newSettings) })
      .where('id', history.id);
