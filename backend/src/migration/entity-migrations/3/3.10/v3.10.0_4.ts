...
interface OldOleDbItemSettings {
...
/**
 * OLEDB stopped talking to the Windows OIBus Agent: it now runs its own local `OIBusOleDbWindows`
 * child process (see backend/native/oledb-windows) instead of a separately installed Windows Service
 * reachable over `agentUrl` — that setting is dropped from both south connectors and history queries.
 *
 * At the same time, OLEDB stopped building CSV content itself (that used to happen inside the .NET
 * agent) and now emits raw `record-list` rows, the same as mysql/postgresql/mssql/oracle/sqlite
 * already do (see 3.10.0_2, which intentionally excluded odbc/oledb from that refactor pending this
 * transport change). This migration applies the exact same item-settings conversion and
 * per-(north|history, item) `record-list-to-csv` transformer attachment 3.10.0_2 used for those five
 * types, scoped to `oledb` only.
 *
 ...
 * reconstructed from that state alone. `agentUrl` is similarly not restorable — down() is a no-op.
 */
export async function up(knex: Knex): Promise<void> {
  ...
  const souths: Array<{ id: string }> = await knex(SOUTH_CONNECTORS_TABLE).select('id').where('type', 'oledb');
 ...
 * idempotent check-then-insert pattern 3.10.0_2 used) so the `north_transformers`/
 * `history_query_transformers` rows inserted below have something to reference. This may already
 * exist if 3.10.0_2 created it for one of the other SQL-family types.
 */
 ...
 */
function toNewItemSettings(oldSettings: OldOleDbItemSettings): Record<string, unknown> {
 ...
 */
function buildTransformerOptions(oldSettings: OldOleDbItemSettings): Record<string, unknown> {
...
async function migrateSouthConnectorItems(
  ...
  for (const item of items) {
    const oldSettings: OldOleDbItemSettings = JSON.parse(item.settings);
...
async function migrateHistoryQueries(knex: Knex, recordListToCsvTransformerId: string): Promise<void> {
  const historyQueries: Array<{ id: string }> = await knex(HISTORY_QUERIES_TABLE).select('id').where('south_type', 'oledb');
