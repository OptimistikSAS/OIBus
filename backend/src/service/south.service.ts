...
const southList: Array<{ class: typeof SouthConnector<SouthSettings, SouthItemSettings>; manifest: SouthConnectorManifest }> = [
...
export default class SouthService {
  ...
  async testSouth<S extends SouthSettings, I extends SouthItemSettings>(
    ...
    const SouthConnector = southList.find(connector => connector.class.type === testToRun.type);
    if (!SouthConnector) {
      throw Error(`South connector of type ${testToRun.type} not installed`);
    }
    const south = new SouthConnector.class(
      testToRun,
  ...
  async testSouthItem<S extends SouthSettings, I extends SouthItemSettings>(
    ...
    const SouthConnector = southList.find(connector => connector.class.type === testConnectorToRun.type);
    if (!SouthConnector) {
      throw Error(`South connector of type ${testConnectorToRun.type} not installed`);
    }
    const south = new SouthConnector.class(
      testConnectorToRun,
      async (_southId: string, _content: OIBusContent): Promise<void> => Promise.resolve(),
  ...
  async importItems<I extends SouthItemSettings>(southConnectorId: string, items: Array<SouthConnectorItemCommandDTO<I>>) {
    ...
    for (const itemCommand of items) {
      await this.validator.validateSettings(manifest.items.settings, itemCommand.settings);
      const southItemEntity = {} as SouthConnectorItemEntity<I>;
      await copySouthItemCommandToSouthItemEntity(southItemEntity, itemCommand, null, manifest, this.encryptionService, scanModes);
...
export const toSouthConnectorDTO = <S extends SouthSettings, I extends SouthItemSettings>(
  entity: SouthConnectorEntity<S, I>,
  manifest: SouthConnectorManifest,
  encryptionService: EncryptionService
): SouthConnectorDTO<S, I> => {
  // TODO : do not pass manifest in argument
  return {
    id: entity.id,
    name: entity.name,
    type: entity.type,
    description: entity.description,
    enabled: entity.enabled,
    sharedConnection: entity.sharedConnection,
    settings: encryptionService.filterSecrets<S>(entity.settings, manifest.settings),
    history: {
      maxInstantPerItem: entity.history.maxInstantPerItem,
      maxReadInterval: entity.history.maxReadInterval,
      readDelay: entity.history.readDelay,
      overlap: entity.history.overlap
    },
    items: entity.items.map(item => toSouthConnectorItemDTO<I>(item, manifest, encryptionService))
  };
};
