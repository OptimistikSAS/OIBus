  ...
  describe('testOIAnalyticsConnection', () => {
    ...
    it('should succeed when API returns 200', async () => {
      ...
      expect(HTTPRequest).toHaveBeenCalledWith(
        expect.objectContaining({ href: 'http://localhost:4200/api/oianalytics/oibus/status' }),
        expect.anything()
      );
    });
    ...
    it('should use specific settings host when useOIAnalyticsRegistration is false', async () => {
      ...
      expect(HTTPRequest).toHaveBeenCalledWith(
        expect.objectContaining({ href: 'https://spec-host/api/oianalytics/oibus/status' }),
        expect.anything()
      );
    });
