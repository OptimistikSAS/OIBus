...
async function generateSettingsInterfaces() {
  ...
  openSync(path.resolve(TRANSFORMER_SETTINGS_DESTINATION_PATH), 'w');
...
/**
 * This function is used to build the transformer settings interface from the various manifests to
 * enforce type safety when using those settings in the transformers.
...
async function generateSettingsInterfacesForTransformers() {
  ...
  buildTypescriptFile(settingsTypesToGenerate, 'Transformer');
}
...
function buildTypescriptFile(typesToGenerate: TypeGenerationDescription, connectorType: ConnectorType | 'Transformer') {
  const path =
    connectorType === 'South'
      ...
      : connectorType === 'North'
        ? NORTH_SETTINGS_DESTINATION_PATH
        : TRANSFORMER_SETTINGS_DESTINATION_PATH;
  ...
  typesToGenerate.settingsInterfaces.forEach(interfaceToWrite => {
    if (interfaceToWrite.attributes.length > 0) {
      appendFileSync(path, `export interface ${interfaceToWrite.name} {\n`);
      interfaceToWrite.attributes.forEach(attribute => {
        writeAttribute(path, attribute);
      });
      ...
      appendFileSync(path, `export type ${interfaceToWrite.name} = object;\n\n`);
    }
  });

  const types = typesToGenerate.settingsInterfaces.map(i => `  | ${i.name}`).join('\n');
  const unionTypeName = connectorType === 'Transformer' ? 'TransformerSettings' : `${connectorType}Settings`;
  appendFileSync(path, `export type ${unionTypeName} =\n`);
...
/**
 * Generate the appropriate types for the given transformer manifest file
...
function buildSouthInterfaceName(connectorId: string, itemInterface: boolean): string {
  ...
  switch (connectorId) {
    ...
    case 'opc':
      return `SouthOPC${prefix}Settings`;
    case 'osisoft-pi':
