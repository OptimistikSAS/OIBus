...
export default class LogServiceMock {
  search = mock.fn((_searchParams: LogSearchParam): Page<OIBusLog> => ({
    content: [],
    size: 50,
    number: 0,
    totalElements: 0,
  ...
  suggestGroups = mock.fn((_name: string): Array<Group> => []);
  getGroupById = mock.fn((_groupId: string): Group => ({}) as Group);
}
