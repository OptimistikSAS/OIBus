...
function toOPCUASecurityMode(securityMode: SouthOPCUASettingsConnectionSettingsSecurityMode) {
  switch (securityMode) {
    case 'none':
      return 1;
    case 'sign':
...
function toOPCUASecurityPolicy(securityPolicy: SouthOPCUASettingsConnectionSettingsSecurityPolicy | null | undefined) {
  ...
  implements QueriesHistory, QueriesLastPoint, QueriesSubscription, SharableConnection<ClientSession>
{
  ...
  override async disconnect(error?: Error): Promise<void> {
    ...
    if (this.client && !connectionService.isConnectionUsed('south', this.connector.id, this.connector.id)) {
      if (!this.connector.settings.sharedConnection) {
