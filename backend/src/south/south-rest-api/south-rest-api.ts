...
import { SouthRestAPISettings, SouthRestAPIItemSettings, DateTimeFields } from '../../../shared/model/south-settings.model';
...
export default class SouthRestAPI extends SouthConnector<SouthRestAPISettings, SouthRestAPIItemSettings> implements QueriesHistory {
  ...
  override async testConnection(): Promise<void> {
    ...
    const testEndpoint = this.connector.settings.testEndpoint || '/';
    const testMethod = this.connector.settings.testMethod || 'GET';
    const successCode = this.connector.settings.successCode || 200;
  ...
  async queryData(
    ...
    if (item.settings.body && ['POST', 'PUT', 'PATCH'].includes(item.settings.method || 'GET')) {
  ...
  parseData(
    ...
    const dateTimeFields: Array<DateTimeFields> = item.settings.dateTimeFields!;
