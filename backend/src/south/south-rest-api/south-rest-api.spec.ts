...
describe('SouthRestAPI connector', () => {
  ...
  beforeEach(() => {
    ...
    south = new SouthRestAPI(configuration, addContentCallback, southCacheRepository, logger, 'cacheFolder', certificateRepository);
  });
  ...
  it('should fall back to default test settings when optional fields are missing', async () => {
    const configuration = createConfiguration();
    configuration.settings.testEndpoint = '';
    configuration.settings.testMethod = undefined as unknown as 'GET';
    configuration.settings.successCode = 0;
    configuration.settings.authentication = 'none';
    const instance = new SouthRestAPI(
      configuration,
      addContentCallback,
