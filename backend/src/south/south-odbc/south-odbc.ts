...
import { convertDateTimeToInstant, convertDelimiter, createFolder, formatInstant, logQuery, persistResults, generateCsvContent, generateFilenameForSerialization } from '../../service/utils';
...
export default class SouthODBC extends SouthConnector<SouthODBCSettings, SouthODBCItemSettings> implements QueriesHistory {
  ...
  async queryRemoteAgentData(item: SouthConnectorItemDTO<SouthODBCItemSettings>, startTime: Instant, endTime: Instant, test? : boolean): Promise<string | Record<string, any>[]> { // test is here in case we are testing items 
  ...
  async queryOdbcData(item: SouthConnectorItemDTO<SouthODBCItemSettings>, startTime: Instant, endTime: Instant, test?: boolean): Promise<string | Record<string, any>[]> { // test is here in case we are testing items 
    if (!odbc) {
      throw new Error('odbc library not loaded');
    }
