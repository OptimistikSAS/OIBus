...
import { compress, checkAge } from '../../service/utils';
...
describe('SouthFolderScanner', () => {
  ...
  describe('testConnection', () => {
    it('should pass testConnection if folder exists, is readable, and is a directory', async () => {
      await expect(south.testConnection()).resolves.toBeUndefined();
  ...
  describe('testItem', () => {
    it('should successfully test an item and return matching files as time-values', async () => {
      jest.spyOn(south, 'testConnection').mockResolvedValueOnce(undefined);
      (fs.readdir as jest.Mock).mockResolvedValueOnce(['file1.csv', 'ignore.txt']);

      const result = await south.testItem(configuration.items[0], { history: undefined });

      expect(fs.readdir).toHaveBeenCalledWith(path.resolve('inputFolder'));
      expect(result.type).toBe('time-values');
      expect((result.content as Array<OIBusTimeValue>).length).toBe(1);
      expect((result.content as Array<OIBusTimeValue>)[0].pointId).toBe('item1');
      expect((result.content as Array<OIBusTimeValue>)[0].data.value).toBe('file1.csv');
    });

    it('should filter out files that fail checkAge', async () => {
      jest.spyOn(south, 'testConnection').mockResolvedValueOnce(undefined);
      (fs.readdir as jest.Mock).mockResolvedValueOnce(['file1.csv', 'file2.csv']);

      // First file passes, second fails
      (checkAge as jest.Mock).mockReturnValueOnce(true).mockReturnValueOnce(false);
