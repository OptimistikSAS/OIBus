...
## Transmission Formats {#transmission-formats}
...
This page's byte-size formulas model CSV files and the `{timestamp, pointId, value}` JSON shape used by the
connectors above. They do not apply to **Modbus**, **MQTT**, or **OPC UA** North connectors, which write
directly to their industrial protocol (register writes, topic publishes, node writes) using a different,
protocol-specific cache format — nor to the **Console** North connector, which only logs to stdout and does
not transmit or buffer data for an outage at all.
:::

## CSV Format Estimations {#csv-format-estimations}
...
### 3. Column-Row Format {#3-column-row-format}
...
## JSON Payload Estimation {#json-payload-estimation}
...
## Format Efficiency Summary {#format-efficiency-summary}
...
## Cache Sizing Calculator {#cache-sizing-calculator}
...
## Effect of Compression {#effect-of-compression}
