...
- The default instance (`-n` omitted, or `-n oibus`) keeps the classic `oibus` service name.
- Any other name creates its own `oibus-<name>` service (letters, digits, dots, hyphens and
...
2. Stop the service: `sudo systemctl stop oibus` (or `oibus-<name>` for a named instance)
3. Replace the binary files in the installation directory with the new ones.
4. Start the service: `sudo systemctl start oibus` (or `oibus-<name>` for a named instance)
...
The data folder is **not** removed automatically if you answer anything other than **Y**/**y**.
:::
