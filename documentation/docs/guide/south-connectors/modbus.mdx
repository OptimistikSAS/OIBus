...
| **Retry interval**     | Time to wait (in milliseconds) between reconnections after a connection failure.                                                                                                  | `10000`         |
...
| Setting                  | Description                                                                                                                                                                                                                                             | Example Value    |
| ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------- |
| **Group batch queries?** | Enable to merge items of the same Modbus type into as few requests as possible, instead of issuing one request per item. Disabled by default.                                                                                                           | Enabled/Disabled |
| **Grouping gap**         | _(Only shown when **Group batch queries?** is enabled)_ Maximum address gap (in registers) allowed when merging non-consecutive items of the same type into a single Modbus request. Set to `0` to batch only strictly consecutive addresses (default). | `0` – `100`      |

With **Group batch queries?** disabled (the default), OIBus issues one Modbus request per item — no merging at all.
Enabling it makes OIBus group items of the same Modbus type (coil, discrete input, input register, holding register)
whose addresses are consecutive into a single request, reducing round-trips to the device.
...
| Data type              | Relevant for _holding registers_ or _input registers_. Defines the type of data fetched from the register (Bit, UInt16, Int16, UInt32, Int32, Big UInt64, Big Int64, Float, Double).  | `UInt16`           |
