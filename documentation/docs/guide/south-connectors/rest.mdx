...
variables (see [Time Variables](#time-variables)) to inject the query's time window directly into a parameter value.
...
#### Date/Time Format for Query Parameters {#datetime-format-for-query-parameters}
...
### Tracking the Maximum Instant {#tracking-the-maximum-instant}
...
#### Tracking Instant Date/Time Format {#tracking-instant-datetime-format}
...
### Shifting a Time Variable

Append `.add(amount, unit)` to `@StartTime` or `@EndTime` to shift it by a fixed offset before it is formatted.
`amount` is an integer (negative to shift backwards) and `unit` is one of `millisecond(s)`, `second(s)`,
`minute(s)`, `hour(s)`, or `day(s)`.

| Example                        | Meaning                                    |
| ------------------------------- | ------------------------------------------- |
| `@StartTime.add(-1, 'day')`     | One day before the start of the interval.   |
| `@EndTime.add(30, 'minutes')`   | Thirty minutes after the end of the interval. |

This is useful for APIs that expect a small buffer or lag around the actual query window, for example
`{"from": "@StartTime.add(-5, 'minutes')", "to": "@EndTime"}`.
