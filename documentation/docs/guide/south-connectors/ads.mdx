...
#### Local AMS Server (TwinCAT Runtime) {#local-ams-server-twincat-runtime}
...
| Port    | Communication endpoint for the PLC.                                                     | `851`           |
...
#### Remote AMS Server {#remote-ams-server}
...
<div style={{ textAlign: 'center' }}>![TwinCAT Static Routes tool](@site/static/img/guide/south/ads/installation-ads-distant.png)</div>

<div style={{ textAlign: 'center' }}>![Add a TwinCAT Static Route](@site/static/img/guide/south/ads/routes.png)</div>
...
#### Structure Filtering {#structure-filtering}

| Setting        | Description                                                                               | Example Value     |
| -------------- | ----------------------------------------------------------------------------------------- | ----------------- |
| Structure Name | Name of the structure to filter.                                                          | `MyStructure`     |
| Fields to Keep | Comma-separated list of fields to retrieve from the structure, or `*` to keep all fields. | `MyDate,MyNumber` |
...
:::tip Keep every field
Set **Fields to Keep** to `*` to keep all fields of the structure instead of listing them individually.
:::
