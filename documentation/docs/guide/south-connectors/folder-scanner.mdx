...
| Max files per scan | Maximum number of files to retrieve per scan. Set to **0** for no limit. When reached, remaining files are retrieved in the next scan.                                                                                         | `0` (no limit), `100`       |
| Max size per scan  | Maximum total size (in MB) of files to retrieve per scan. Set to **0** for no limit. When reached, remaining files are retrieved in the next scan.                                                                             | `0` (no limit), `500`       |
