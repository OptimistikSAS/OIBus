...
Query Microsoft SQL Server™ (MSSQL) databases.

## Specific settings

When configuring the SQL connector to send SQL queries to a Microsoft SQL Server, you will need to provide the following details:
...
- **Use encryption**: Option to encrypt the data between the database and OIBus (note that encryption can potentially overload the server).
...
:::tip Database access
It is strongly advised to use a read-only user when connecting to the database for security and data integrity
purposes.
:::
