...
![MSSQL -> Azure Blob use case](@site/static/img/use-cases/mssql-azure-blob.svg)
...
![MSSQL settings](@site/static/img/use-cases/mssql-settings.png)
...
#### Without variable {#without-variable}
...
![MSSQL item without variable](@site/static/img/use-cases/mssql-item-1.png)

</div>

#### With variables {#with-variables}

The `@StartTime` variable captures the last maximum instant retrieved from a previous query. For the very first query, before any instant
has been tracked, OIBus looks back by the **Max read interval** setting (in seconds) from the current time — or 1 hour if Max read interval
is left at `0`. The `@EndTime` variable is set to the current time, or to the sub-interval end when the query is split by **Max read interval**.
...
- **Start time offset** (in milliseconds): added to the start of the query window (`@StartTime`). A negative value moves the start earlier,
  to capture late-arriving data — this is the old "Overlap" behavior.
- **End time offset** (in milliseconds): added to the end of the query window (`@EndTime`). A negative value pulls the end in earlier; a
  positive value extends it later.
- **Recovery strategy**: order in which OIBus catches up a backlog of sub-intervals — `From oldest to newest` (default) or
  `From newest to oldest`.

<div style={{ textAlign: 'center' }}>

![MSSQL history settings](@site/static/img/use-cases/mssql-history-settings.png)

</div>

:::caution When to use Start time offset?
A negative **Start time offset** is beneficial when certain retrieved data exhibit latency and are stored after the query has been executed.
In such cases, it allows for a slight extension of the requested interval backwards, so that late-arriving rows are not missed.
...
![MSSQL item with variable](@site/static/img/use-cases/mssql-item-2.png)
...
![Azure Blob settings](@site/static/img/use-cases/azure-blob-settings.png)
