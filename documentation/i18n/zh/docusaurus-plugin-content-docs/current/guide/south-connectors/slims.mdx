...
thus reducing the load on both the server and the network (as explained in the [big queries section](#拆分大型查询)).
...
- **Reference field**: Use this field as reference for the next @StartTime value (see [query variables](#查询变量)).
