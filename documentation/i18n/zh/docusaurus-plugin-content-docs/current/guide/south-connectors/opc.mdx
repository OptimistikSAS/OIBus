...
# OPCHDA
OPCDA和OPCHDA是由[OPC基金会](https://opcfoundation.org/)开发的工业界使用的通信协议。这项技术已被OPCUA取代，但在工业界仍广泛使用。要在OIBus中使用OPCUA，请参阅[OPCUA连接器文档](./opcua.mdx)。
...
HDA模块也可以作为独立使用，以命令行方式执行OPC历史数据提取。参阅[OPC代理文档](../oibus-agent/opc.mdx)以独立使用该模块，并查看COM/DCOM设置文档，以正确安装该模块。
