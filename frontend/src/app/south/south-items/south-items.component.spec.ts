...
describe('SouthItemsComponent CSV Import Tests', () => {
  ...
  describe('with saveChangesDirectly = true', () => {
    ...
    it('should handle CSV validation error and show error message', () => {
      const validationError: CsvValidationError = {
        missingHeaders: ['name'],
        extraHeaders: ['invalid_column'],
        expectedHeaders: ['name', 'enabled', 'scanMode', 'settings_query'],
        actualHeaders: ['enabled', 'scanMode', 'settings_query', 'invalid_column']
      };

      const mockModalRef = {
        componentInstance: { expectedHeaders: [] },
        result: of({ validationError })
      };
      modalService.open.and.returnValue(mockModalRef as any);
      ...
      const msg = notificationService.error.calls.mostRecent().args[0] as string;
      expect(msg).toContain('Missing columns: name');
      expect(msg).toContain('Extra columns: invalid_column');
      expect(msg).toContain('Expected format: name, enabled, scanMode, settings_query');
      expect(msg).toContain('Actual format: enabled, scanMode, settings_query, invalid_column');
    });

    it('should call checkImportItems when validation passes', () => {
    ...
    it('should open ImportSouthItemsModalComponent after successful check', () => {
      ...
      // first open → ImportItemModalComponent
      // second open → ImportSouthItemsModalComponent
  ...
  describe('CSV validation error message building', () => {
    beforeEach(() => {
      tester.componentInstance.saveChangesDirectly = true;
      tester.componentInstance.manifest.items.settings = [
        { key: 'query', type: 'OibText', translationKey: 'south.items.mssql.query', displayInViewMode: true },
        { key: 'timeout', type: 'OibNumber', translationKey: 'south.items.mssql.query', displayInViewMode: true },
        { key: 'enabled', type: 'OibCheckbox', translationKey: 'south.items.mssql.query', displayInViewMode: true }
      ];
      tester.detectChanges();
    });

    it('should build comprehensive error message for missing and extra headers', () => {
      const validationError: CsvValidationError = {
        missingHeaders: ['name', 'enabled'],
        extraHeaders: ['invalid_col1', 'invalid_col2'],
        expectedHeaders: ['name', 'enabled', 'scanMode', 'settings_query'],
        actualHeaders: ['scanMode', 'settings_query', 'invalid_col1', 'invalid_col2']
      };
      ...
      const msg = notificationService.error.calls.mostRecent().args[0] as string;
      expect(msg).toContain('Missing columns: name, enabled');
      expect(msg).toContain('Extra columns: invalid_col1, invalid_col2');
      expect(msg).toContain('Expected format: name, enabled, scanMode, settings_query');
      expect(msg).toContain('Actual format: scanMode, settings_query, invalid_col1, invalid_col2');
    });

    it('should build error message with only missing headers', () => {
      const validationError: CsvValidationError = {
        missingHeaders: ['name'],
        extraHeaders: [],
        expectedHeaders: ['name', 'enabled'],
        actualHeaders: ['enabled']
      };
      modalService.open.and.returnValue({
        componentInstance: { expectedHeaders: [] },
        result: of({ validationError })
      ...
      const msg = notificationService.error.calls.mostRecent().args[0] as string;
      expect(msg).toContain('Missing columns: name');
      expect(msg).not.toContain('Extra columns');
    });

    it('should build error message with only extra headers', () => {
      const validationError: CsvValidationError = {
        missingHeaders: [],
        extraHeaders: ['invalid_col'],
        expectedHeaders: ['name', 'enabled'],
        actualHeaders: ['name', 'enabled', 'invalid_col']
      };
      modalService.open.and.returnValue({
        componentInstance: { expectedHeaders: [] },
        result: of({ validationError })
      ...
      const msg = notificationService.error.calls.mostRecent().args[0] as string;
      expect(msg).toContain('Extra columns: invalid_col');
      expect(msg).not.toContain('Missing columns');
    });
  });
