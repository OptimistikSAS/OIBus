...
describe('ItemTestTableResultComponent', () => {
  ...
  it('should provide supported display modes', () => {
    ...
    expect(
      tester.componentInstance.testedComponent().getSupportedDisplayModes({ type: 'any', filePath: 'test.csv' } as OIBusContent)
    ).toEqual(null);

    // any (generic) -> nothing
    expect(
      tester.componentInstance.testedComponent().getSupportedDisplayModes({ type: 'any', filePath: 'test.txt' } as OIBusContent)
    ).toEqual(null);
