...
describe('SouthConnectorService', () => {
  ...
  test('should check import items from rows', () => {
  ...
  test('should check import items from rows without a match key', () => {
    const rows = [{ name: 'item1' }];
    let actualImportation = false;

    service.checkImportItemsFromRows('southType', [], rows).subscribe(() => {
      actualImportation = true;
    });

    const testRequest = http.expectOne({ method: 'POST', url: '/api/south/southType/items/import/check' });
    expect(testRequest.request.body.has('matchKey')).toBe(false);
    testRequest.flush(true);

    expect(actualImportation).toBe(true);
  });

  test('should import items with a match key and row resolutions', () => {
    const expectedFormData = new FormData();
    expectedFormData.set('items', new Blob([JSON.stringify([])], { type: 'application/json' }), 'items.json');
    expectedFormData.set('deleteItemsNotPresent', 'false');
    expectedFormData.set('matchKey', 'name');
