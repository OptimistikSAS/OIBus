...
class ExploreTreeComponentTester {
  ...
  readonly selectButtons = this.root.getByCss('.explore-select');
}
...
describe('ExploreTreeComponent', () => {
  ...
  test('should not render a Select action on any node by default (read-only browsing)', () => {
    southConnectorService.startExplore.mockReturnValue(
      of({
        sessionId: 'sessionId',
        entries: [{ id: 'parent', name: 'Parent', metadata: { type: 'Object' }, hasChildren: true }]
      })
    );
    tester.component.prepare(southConnector.id, southConnector.settings, southConnector.type);
    tester.fixture.detectChanges();

    expect(tester.selectButtons.elements().length).toBe(0);
  });

  test('should render a Select action on expandable nodes only, and emit nodeSelected when clicked', () => {
    southConnectorService.startExplore.mockReturnValue(
      of({
        sessionId: 'sessionId',
        entries: [
          { id: 'parent', name: 'Parent', metadata: { type: 'Object' }, hasChildren: true },
          { id: 'leaf', name: 'Leaf', metadata: { type: 'Variable' }, hasChildren: false }
        ]
      })
    );
    ...
    // Only the expandable node gets a Select action - picking a leaf as a discovery root would
    // scope the walk to nothing.
    expect(tester.selectButtons.elements().length).toBe(1);
