...
const ConfigService = require('./config.service.class')
...
jest.mock('./EncryptionService.class', () => ({ getInstance: () => ({ encryptSecrets: (password) => password }) }))
...
describe('Config services', () => {
  beforeEach(() => {
    ...
    service = new ConfigService(configFilePath, cacheFolder)
  })
  ...
  it('should properly initialized service with default conf', async () => {
    ...
    expect(fs.readFile).toHaveBeenCalledWith(`${__dirname}/../config/defaultConfig.json`, 'utf8')
