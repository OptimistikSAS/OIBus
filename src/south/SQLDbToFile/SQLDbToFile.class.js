...
class SQLDbToFile extends ProtocolHandler {
  ...
  constructor(dataSource, engine) {
    ...
    if (moment.tz.zone(timezone)) {
      ...
      logger.error(`Invalid timezone supplied: ${this.timezone}`, this.logSource)
    }
  ...
  async onScan(_scanMode) {
    ...
    try {
      switch (this.driver) {
        ...
        case 'oracle':
        default:
          logger.error(`Driver ${this.driver} not supported by ${this.dataSource.dataSourceId}`, this.logSource)
          result = []
      }
    } catch (error) {
    ...
    logger.debug(`Found ${result.length} results`, this.logSource)
    ...
    if (result.length > 0) {
      ...
      if (this.lastCompletedAt) {
        logger.debug(`Updating lastCompletedAt to ${this.lastCompletedAt}`, this.logSource)
      } else {
        logger.debug('lastCompletedAt not used', this.logSource)
      }
      ...
      if (csvContent) {
        ...
        try {
          logger.debug(`Writing CSV file at ${filePath}`, this.logSource)
  ...
  async getDataFromMSSQL() {
    ...
    logger.debug(`Executing "${adaptedQuery}"`, this.logSource)
