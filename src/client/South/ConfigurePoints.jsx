...
const ConfigurePoints = () => {
  ...
  const filteredPoints = filterText
    ? points.filter((point) => {
      ...
      return (
        Object.values(filterableAttributes).findIndex((element) => element.toString().toLowerCase().includes(filterText.toLowerCase())) >= 0
      )
    })
    : points
  ...
  const tableHelps = pointsWithHelp.length > 0
