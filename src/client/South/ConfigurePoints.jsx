...
const ConfigurePoints = () => {
  ...
  const points = pointsOrdered
    .slice()
    .map((item, index) => ({ virtualIndex: index, ...item }))
    .reverse();
  ...
  const filteredPoints = filterText
    ? points.filter((point) => {
        ...
        return (
          Object.values(filterableAttributes).findIndex((element) =>
            element.toString().toLowerCase().includes(filterText.toLowerCase())
          ) >= 0
        );
      })
    : points;
  ...
  const findIndexBasedOnVirtualIndex = (index) => {
    ...
    const indexInTable = points.findIndex(
      (point) => point.virtualIndex === pointToOperate.virtualIndex
    );
  ...
  const handleDelete = (index) => {
    dispatchNewConfig({
      type: "deleteRow",
      name: `south.dataSources.${dataSourceIndex}.points.${findIndexBasedOnVirtualIndex(
        index
      )}`,
    });
  };
  ...
  const onChange = (name, value, validity) => {
    ...
    const pathWithPageOffset = name.replace(
      /[0-9]+/g,
      `${findIndexBasedOnVirtualIndex(index)}`
    );
    dispatchNewConfig({
      type: "update",
  ...
  const pointsWithHelp = Object.entries(ProtocolSchema.points).filter(
    ([name, value]) => name && value.help
  );
  const tableHelps =
    pointsWithHelp.length > 0 &&
    pointsWithHelp.map(([name, value]) => (
      <div key={name}>
  ...
  const tableHeaders = Object.entries(ProtocolSchema.points).map(
    ([name, value]) => value.label || humanizeString(name)
  );
  ...
  const tableRows = pagedPoints.map((point, index) =>
    Object.entries(ProtocolSchema.points).map(([key, value]) => {
      ...
      if (ProtocolSchema.points[key].unique) {
        ...
        const pointIds = points
          .filter(
            (_point) =>
              _point.virtualIndex !==
              filteredPoints[indexOffset + index].virtualIndex
          )
          .map((p) => p[key]);
        const oldValid = rest.valid.bind({});
        rest.valid = (val) =>
          oldValid(val) ||
          validation.points.isUnique(val, pointIds) ||
          validation.points.noUnintendedTrailingSpaces(val);
      }
              ...
              {(confirm) => (
                <div>
                  <Button
                    className="inline-button"
                    color="danger"
                    onClick={confirm(() => handleDeleteAllPoint())}
                  >
                    Delete All Points
                  </Button>
                </div>
