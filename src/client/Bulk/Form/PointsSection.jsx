...
const PointsSection = ({ bulk, bulkIndex }) => {
  ...
  const handleDelete = (index) => {
    dispatchNewHistoryConfig({ type: 'deleteRow', name: `${bulkIndex}.points.${findIndexBasedOnVirtualIndex(index)}` })
  }
  ...
  const onChange = (name, value, validity) => {
    ...
    dispatchNewHistoryConfig({
      type: 'update',
      name: `${bulkIndex}.${pathWithPageOffset}`,
      value,
      validity,
    })
  }
  ...
  const tableRows = pagedPoints.map((point, index) => Object.entries(ProtocolSchema.points).map(([key, value]) => {
    ...
    return (
      ...
      { name, value: <Control onChange={onChange} name={name} {...rest} southId={dataSource.id} /> }
    )
  }))
