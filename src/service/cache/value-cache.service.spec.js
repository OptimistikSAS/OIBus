...
describe('ValueCache', () => {
  ...
  it('should retry to send values if it fails', async () => {
    ...
    await cache.sendValues()
    expect(logger.debug).toHaveBeenCalledWith('Retrying in 5000 ms. Retry count: 1')
    jest.advanceTimersByTime(settings.retryInterval)
    expect(logger.debug).toHaveBeenCalledWith('Retrying in 5000 ms. Retry count: 2')
    jest.advanceTimersByTime(settings.retryInterval)
    expect(logger.debug).toHaveBeenCalledWith('Retrying in 5000 ms. Retry count: 3')
    jest.advanceTimersByTime(settings.retryInterval)
